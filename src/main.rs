...
fn main() {
    ...
    let file_name = args.next().unwrap_or_else(|| String::from("points_2d.txt"));
    ...
    while !rl_handle.window_should_close() {
...

        {
            ...
            for &(v1, v2, v3) in triangles_draw.iter() {
                draw_handle.draw_triangle(v1, v2, v3, Color::GREEN.fade(0.3));
                if show_triangulation {
                    draw_handle.draw_line_ex(v1, v2, 1.0, Color::GREEN);
                    draw_handle.draw_line_ex(v2, v3, 1.0, Color::GREEN);
                    draw_handle.draw_line_ex(v3, v1, 1.0, Color::GREEN);
                }
            }

            for v in points_draw.windows(2) {
                draw_handle.draw_line_ex(v[0], v[1], 5.0, Color::BLACK);
            }

            for &v in points_draw.iter() {
                draw_handle.draw_circle_v(v, 6.0, Color::BLACK);
            }

            for &(v1, v2) in medial_axis_draw.iter() {
                draw_handle.draw_line_ex(v1, v2, 5.0, Color::GREEN);
                // draw_handle.draw_circle_v(v1, 6.0, Color::GREEN);
                // draw_handle.draw_circle_v(v2, 6.0, Color::GREEN);
            }
        }
