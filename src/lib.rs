...
pub fn triangulate(points: &[Point]) -> Vec<Triangle> {
    ...
    let super_p1 = Point::new(point_id, mid_x - 20.0 * d_max, mid_y - d_max);
    let super_p2 = Point::new(point_id + 1, mid_x, mid_y + 20.0 * d_max);
    let super_p3 = Point::new(point_id + 2, mid_x + 20.0 * d_max, mid_y - d_max);
...
pub fn medial_axis(triangles: &[Triangle]) -> Vec<Edge> {
    ...
    for (_, (t1, t2)) in adjacency {
        if let Some(t2) = t2 {
            axis.push(Edge::new(t1.circumcenter, t2.circumcenter));
        }
    }
